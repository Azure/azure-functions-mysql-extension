    ...
    public class IntegrationTestFixture : IDisposable
    {
        ...
        private void StartFunctionHosts()
        {
            ...
            foreach (SupportedLanguages lang in supportedLanguages)
            {
                ...
                {
                    this.StartHost(Path.Combine(binPath, "MySqlExtensionSamples", "CSharp"), TestUtils.GetPort(lang), this.SampleFunctions);
                    this.StartHost(Path.Combine(binPath), TestUtils.GetPort(lang, true), this.TestFunctions);
                }
                else if (lang == SupportedLanguages.Java)
                {
                    this.StartHost(Path.Combine(binPath, "MySqlExtensionSamples", "Java", "target", "azure-functions", "azure-functions-java-library-mysql-samples"), TestUtils.GetPort(lang), this.SampleFunctions);
                    this.StartHost(Path.Combine(binPath, "..", "..", "..", "Integration", "test-java", "target", "azure-functions", "azure-functions-java-library-mysql-tests"), TestUtils.GetPort(lang, true), this.TestFunctions);
                }
                else if (lang == SupportedLanguages.OutOfProc)
                {
                    this.StartHost(Path.Combine(binPath, "MySqlExtensionSamples", "OutOfProc"), TestUtils.GetPort(lang), this.SampleFunctions);
                    this.StartHost(Path.Combine(binPath, "MySqlExtensionSamples", "OutOfProc", "test"), TestUtils.GetPort(lang, true), this.TestFunctions);
                }
                else
                {
                    this.StartHost(Path.Combine(binPath, "MySqlExtensionSamples", Enum.GetName(typeof(SupportedLanguages), lang)), TestUtils.GetPort(lang), null);
                }
            }
        }
