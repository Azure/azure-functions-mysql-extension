...
{
    public class MySqlTriggerScaleMonitorTests
    {
        /// <summary>
        /// Verifies that the scale monitor descriptor ID is set to expected value.
        /// </summary>
        [Theory]
        [InlineData("testTableName", "testUserFunctionId", "testUserFunctionId-MySqlTrigger-testTableName")]
        [InlineData("тестТаблицаИмя", "тестПользовательФункцияИд", "тестПользовательФункцияИд-MySqlTrigger-тестТаблицаИмя")]
        ...
        {
            IScaleMonitor<MySqlTriggerMetrics> monitor = GetScaleMonitor(tableName, userFunctionId);
        ...
        {
            (IScaleMonitor<MySqlTriggerMetrics> monitor, List<string> _) = GetScaleMonitor();
        ...
        {
            (IScaleMonitor<MySqlTriggerMetrics> monitor, _) = GetScaleMonitor();
        ...
        {
            (IScaleMonitor<MySqlTriggerMetrics> monitor, List<string> logMessages) = GetScaleMonitor();
            ScaleStatusContext context = GetScaleStatusContext(unprocessedChangeCounts, workerCount);
        ...
        {
            (IScaleMonitor<MySqlTriggerMetrics> monitor, _) = GetScaleMonitor(maxChangesPerWorker);
        ...
        public void InvalidUserConfiguredMaxChangesPerWorker(string maxChangesPerWorker)
        {
            ...
            Assert.Throws<InvalidOperationException>(() => new MySqlTriggerListener<object>("testConnectionString", "testTableName", "", "testUserFunctionId", Mock.Of<ITriggeredFunctionExecutor>(), Mock.Of<MySqlOptions>(), mockLogger.Object, mockConfiguration.Object));
        }

        private static IScaleMonitor<MySqlTriggerMetrics> GetScaleMonitor(string tableName, string userFunctionId)
        {
            return new MySqlTriggerScaleMonitor(
                userFunctionId,
                new MySqlObject(tableName),
                "testUserDefinedLeasesTableName",
                "testConnectionString",
                MySqlOptions.DefaultMaxChangesPerWorker,
                Mock.Of<ILogger>());
        }

        private static (IScaleMonitor<MySqlTriggerMetrics> monitor, List<string> logMessages) GetScaleMonitor(int maxChangesPerWorker = MySqlOptions.DefaultMaxChangesPerWorker)
        {
            (Mock<ILogger> mockLogger, List<string> logMessages) = CreateMockLogger();

            IScaleMonitor<MySqlTriggerMetrics> monitor = new MySqlTriggerScaleMonitor(
                "testUserFunctionId",
                new MySqlObject("testTableName"),
                "testUserDefinedLeasesTableName",
                "testConnectionString",
                maxChangesPerWorker,
                mockLogger.Object);
        ...
        private static ScaleStatusContext GetScaleStatusContext(int[] unprocessedChangeCounts, int workerCount)
        {
            ...
            {
                Metrics = unprocessedChangeCounts?.Select((count, index) => new MySqlTriggerMetrics
                {
        ...
        private static Mock<IConfiguration> CreateMockConfiguration(string maxChangesPerWorker = null)
        {
            ...
            mockConfiguration
                .Setup(x => x.GetSection("MySql_Trigger_MaxChangesPerWorker"))
