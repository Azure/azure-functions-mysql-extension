...
namespace Microsoft.Azure.WebJobs.Extensions.MySql
{
    ...
    /// </summary>
    internal class MySqlTriggerScaleMonitor : IScaleMonitor<MySqlTriggerMetrics>
    {
        ...
        {
            return this.GetScaleStatusWithTelemetry(context.WorkerCount, context.Metrics?.Cast<MySqlTriggerMetrics>().ToArray());
        }

        public ScaleStatus GetScaleStatus(ScaleStatusContext<MySqlTriggerMetrics> context)
        {
            return this.GetScaleStatusWithTelemetry(context.WorkerCount, context.Metrics?.ToArray());
        }

        private ScaleStatus GetScaleStatusWithTelemetry(int workerCount, MySqlTriggerMetrics[] metrics)
        {
            ...
            try
            {
                ...
                this._logger.LogInformation($"Getting the scale status for table '{this._userTable.FullName}'");
            }
            catch (Exception ex)
            {
